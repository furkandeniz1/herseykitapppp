...
function processOrder() {
    ...
    if (cardNumber && expiryDate && cvv && cardHolderName) {
        const orderId = 'ORD' + Math.random().toString(36).substr(2, 13).toUpperCase();
        document.getElementById('order-id').textContent = `Sipariş Numarası: ${orderId}`;
        document.getElementById('order-address').textContent = `Adres: ${document.getElementById('address').value}`;
        document.getElementById('order-contact').textContent = `İletişim: ${document.getElementById('email').value} - ${document.getElementById('phone').value}`;
        document.getElementById('order-card-last4').textContent = `Kart Numarası (Son 4 Hane): ${cardNumber.slice(-4)}`;
